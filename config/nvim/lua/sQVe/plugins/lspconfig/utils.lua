...
M.create_root_dir_handler = function(opts)
	...
	return function(filename)
		...
		local manifest = vim.fs.dirname(vim.fs.find("package.json", { path = filename, upward = true })[1])

		if opts.prioritizeManifest and manifest then
			return manifest
		end

		return vim.fs.dirname(vim.fs.find(".git", { path = filename, upward = true })[1]) or manifest
	end
end
...
M.map_diagnostic_keys = function(bufnr)
	...
	map("n", "[d", function()
		vim.diagnostic.jump(vim.tbl_extend("force", shared_diagnostic_opts, { count = -1 }))
...
M.map_lookup_keys = function(bufnr)
	...
	map("n", "gd", Snacks.picker.lsp_definitions, { buffer = bufnr, desc = "Go to definition" })

	map("n", "gD", Snacks.picker.lsp_declarations, { buffer = bufnr, desc = "Go to declaration" })

	map("n", "gI", function()
		Snacks.picker.lsp_implementations({ reuse_win = true })
	end, { buffer = bufnr, desc = "Go to implementation" })

	map("n", "gr", Snacks.picker.lsp_references, { buffer = bufnr, desc = "Go to reference" })

	map("n", "gR", function()
		vim.lsp.buf.rename()
	end, { buffer = bufnr, desc = "Rename symbol" })

	map("n", "gy", Snacks.picker.lsp_type_definitions, { buffer = bufnr, desc = "Go to type definition" })

	map("n", "K", vim.lsp.buf.hover, { buffer = bufnr, desc = "View symbol information" })
end
